...
var vm = function () {
    ...
    self.recoverFavs = function() {
        if (localStorage.getItem("arenas_favs")) {
            favs = JSON.parse(localStorage.getItem("arenas_favs"))
            for (var id in favs) {
    ...
    self.favReady = function() {
        $('[id^="favourite_"]').on('click', e => {
            ...
            localStorage.setItem("arenas_favs", JSON.stringify(favs))
            self.update()
        })
    };
...
function autocomplete() {
    if (localStorage.getItem('arenasJSON') == undefined || localStorage.getItem('arenasJSON') == null) {
        ajaxHelper('http://192.168.160.58/NBA/API/Arenas', 'GET').done(function (data) {
            localStorage.setItem('arenasJSON', JSON.stringify(data.Records))
        })
    }

    var availableTags = JSON.parse(localStorage.getItem('arenasJSON'))
    var values = []
    ...
    $("#tags").autocomplete({
        ...
        select: function (event, ui) {   
            window.location.assign('arenaDetails.html?id=' + ui.item.value.split(",")[0].trim());
        }
    }
    );
}
